	...
	unsigned int len, uint8_t* ecc)
{
	...
	unsigned long m;
	uint32_t w;
	uint32_t* r = (uint32_t*)alloca(sizeof(uint32_t) * (l + 1));
	...
	unsigned int* sol, int nsol)
{
	...
	int rem, c, r, p, k;
	int* param = (int*)alloca(sizeof(int) * m);
...
static int build_deg2_base(struct bch_control* bch)
{
	...
	unsigned int* xi = (unsigned int*)alloca(sizeof(unsigned int) * m);
...
static uint32_t* compute_generator_polynomial(struct bch_control* bch)
{
	...
	g = (gf_poly*)bch_alloc(GF_POLY_SZ(m * t), &err);
...
struct bch_control* init_bch(int m, int t, unsigned int prim_poly)
{
	...
	bch = (bch_control*)kzalloc(sizeof(*bch), GFP_KERNEL);
	if (bch == NULL)
		goto fail;
	...
	bch->cache = (int*)(2 * t * sizeof(*bch->cache), &err);
